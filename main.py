...
def criar_usuario(user: schemas.UserCreate, db: Session = Depends(get_db)):
    if crud.pegar_email(db, email=user.email):
...
def atualizar_usuario(user_id: int, user:schemas.UserUpdate, db: Session = Depends(get_db)):
    db_user = crud.pegar_id(db, user_id)
    if db_user is None:
    ...
    if user.email:
        ...
        if email_existente:
            raise HTTPException(status_code=400, detail="Email já cadastrado")
        return crud.atualizar_usuario(db=db, db_user=db_user, user=user)
...
def excluir_usuario(user_id: int, db: Session = Depends(get_db)):
    ...
    if db_user is None:
        raise HTTPException(status_code=404, detail="Usuário não encontrado")
    crud.excluir_usuario(db=db, db_user=db_user)
